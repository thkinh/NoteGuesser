...
function handleGenerate(){
  if(justRight == false ){
    winStreak = 0;	
    winStreakMessage.textContent = `Winstreak: ${winStreak}`;
  }
  justRight = false;
...
confirmButton.addEventListener("click", () => {
    if(justRight) {
    resultMessage.textContent = `Please regenerate the question`;
    return;
    }
    ...
    resultMessage.textContent = selectedNote == Answer ? `CORRECT` : `WRONG`; //Check the result
